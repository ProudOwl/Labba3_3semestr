...
func TestOpenHash_DeleteLogic(t *testing.T) {
	...
	if hash.Find("b") != "" {
		t.Error("Should not find deleted key 'b'")
	}
...
func TestOpenHash_DeleteThenFind(t *testing.T) {
	...
	// Проверяем, что все ключи находятся
	for i, key := range keys {
