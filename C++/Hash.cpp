...
Hash::Hash(int buckCount) : table(nullptr), cap(buckCount)
{
    if(cap < 1) cap=1;
    table = new Node*[cap];
...
auto Hash::erase(const string& key) -> bool
{
    int idx = hash_str(key, cap);
...
auto Hash::find(const string& key) -> string
{
    int idx = hash_str(key, cap);
    Node* cur = table[idx];
...
void Hash::clearAndResize(int newCap) {
    for(int i = 0; i < cap; i++) {
        Node* cur = table[i];
        while(cur != nullptr) {
            Node* nxt = cur->next;
