...
TEST(DoublyListTest, Branch_Killer) {
    DoublyList d;
...
TEST(DoublyListTest, Coverage_Empty_Ops) {
    DoublyList d;
    d.delHead();
    d.delTail();
    d.delByVal("A");
...
TEST(DoublyListTest, Coverage_SingleElement_Transitions) {
    DoublyList d;
...
TEST(DoublyListTest, Coverage_AddBefore_AddAfter_Complex) {
    DoublyList d;
    d.addTail("A");
    d.addTail("C");
...
TEST(DoublyListTest, Coverage_DelByVal_Branches) {
    DoublyList d;
    d.addTail("1");
    d.addTail("2");
    d.addTail("3");
...
TEST(DoublyListTest, Coverage_DelAfter_DelBefore_Boundary) {
    DoublyList d;
    d.addTail("A");
    d.addTail("B");
...
TEST(DoublyListTest, Coverage_IO) {
    DoublyList d;
    DoublyListSerializer::saveToFile(d, ""); 
    DoublyListSerializer::saveToBinaryFile(d, "");
    
    DoublyListSerializer::loadFromFile(d, "missing.txt");
    DoublyListSerializer::loadFromBinaryFile(d, "missing.bin");
    ...
    DoublyListSerializer::loadFromBinaryFile(d, "bad.bin");
    remove("bad.bin");
}
// проверка вывода обратного списка с одним элементом
TEST(DoublyListTest, ReadBackward_SingleElement) {
    DoublyList d;
    d.addTail("single");
...
TEST(DoublyListTest, ReadBackward_MultipleElements) {
    DoublyList d;
    d.addTail("first");
    d.addTail("second");
    d.addTail("third");
...
TEST(DoublyListTest, ReadBackward_EmptyList) {
    DoublyList d;
...
TEST(DoublyListTest, ReadBackward_TwoElements) {
    DoublyList d;
    d.addTail("first");
    d.addTail("last");
...
TEST(DoublyListTest, ReadBackward_AfterDeleteOperations) {
    DoublyList d;
    d.addTail("A");
    d.addTail("B");
    d.addTail("C");
...
TEST(DoublyListTest, ReadBackward_SpecialCharacters) {
    DoublyList d;
...
TEST(DoublyListTest, ReadBackward_AfterClear) {
    DoublyList d;
    d.addTail("A");
    d.addTail("B");
...
TEST(DoublyListTest, ReadBackward_AfterAddHead) {
    DoublyList d;
    d.addHead("C");
    d.addHead("B");
    d.addHead("A");
...
TEST(DoublyListTest, ReadBackward_MixedOperations) {
    DoublyList d;
    d.addTail("A");
    d.addHead("Start");
    d.addTail("Z");
